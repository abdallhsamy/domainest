...
impl CaddyManager {
  ...
  pub fn write_managed_caddyfile(&self, projects: &[Project], tls_paths: &TlsPathLookup) -> AppResult<PathBuf> {
...
fn wait_for_admin_ready(addr: &str, timeout: Duration) -> AppResult<()> {
  ...
  Err(AppError::ToolFailed {
    tool: "caddy admin".to_string(),
    message: format!("admin endpoint did not become ready at {addr} within {}ms", timeout.as_millis()),
  })
}
